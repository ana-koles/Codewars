...
The first and last elements of the array will not be considered as peaks (in the context
    of a mathematical function, we don't know what is after and before and therefore, we
    don't know if it is a peak or not).

Also, beware of plateaus !!! [1, 2, 2, 2, 1] has a peak while [1, 2, 1, 2, 1] and [1, 2, 2, 2, 2]
    ...
    console.log(arr)
      ...
      arr.forEach((item, index, arr) => {
          if (index === 0 || index === (arr.length - 1)) {
              ...
              if (item > arr[index -1] && item > arr[index + 1] && item === arr[index + 2]) {
                  plateaus = true;
              } else if (item < arr[index -1] && item < arr[index + 1] && item === arr[index + 2]) {
                  plateaus = true;
              } else {
                  plateaus = false;
              }
          ...
          if (index === arr.length - 2){
              if (item > arr[index -1] && item > arr[index + 1] && plateaus === true) {
                  plateaus = true;
              } else if (item < arr[index -1] && item < arr[index + 1] && plateaus === true) {
                  plateaus = true;
              } else {
                  plateaus = false;
              }
          ...
          if (index === 0 || index === (arr.length - 1)) {
             return;
              ...
              if (item > arr[index -1] && item > arr[index + 1]) {
                  obj.pos.push(index);
                  obj.peaks.push(item);
              } else if (item > arr[index -1] && item === arr[index + 1] && item > arr[arr.length - 1]) {
                  obj.pos.push(index);
                  obj.peaks.push(item);
              }
          }
